...
const randomUserMock = [
    ...
    },
    {
        ...
        dob: { date: "1964-03-23T21:11:35.935Z", age: 33 },
...
const normalizeUser = (user) => {
    ...
    return ({
        ...
        postcode: ((_g = user.location) === null || _g === void 0 ? void 0 : _g.postcode) || "-",
        coordinates: ((_h = user.location) === null || _h === void 0 ? void 0 : _h.coordinates) || { latitude: "-", longitude: "-" },
        timezone: ((_j = user.location) === null || _j === void 0 ? void 0 : _j.timezone) || { offset: "-", description: "-" },
        email: user.email || "-",
        b_date: ((_k = user.dob) === null || _k === void 0 ? void 0 : _k.date) || "-",
        age: ((_l = user.dob) === null || _l === void 0 ? void 0 : _l.age) || "-",
        phone: user.phone || "-",
        picture_large: ((_m = user.picture) === null || _m === void 0 ? void 0 : _m.large) || "-",
        picture_thumbnail: ((_o = user.picture) === null || _o === void 0 ? void 0 : _o.thumbnail) || "-",
        id: ((_p = user.login) === null || _p === void 0 ? void 0 : _p.uuid) || "-",
        favorite: false,
...
// Додаємо кнопку застосування фільтрів
const filterButton = document.createElement("button");
filterButton.textContent = "Apply Filters";
filterButton.id = "applyFilters";
(_a = document.querySelector(".filters")) === null || _a === void 0 ? void 0 : _a.appendChild(filterButton);
...
const filterUsers = () => {
    ...
    let filteredUsers = users.filter((user) => {
        if (ageFilter === "18-31" && !(user.age >= 18 && user.age <= 31))
            return false;
        if (ageFilter === "32-45" && !(user.age >= 32 && user.age <= 45))
            return false;
        if (ageFilter === "46+" && !(user.age >= 46))
            return false;
        const regions = {
        ...
        if (regionFilter !== "Other" && !regions[regionFilter].includes(user.country))
            return false;
        if (sexFilter !== "Other" && user.gender !== sexFilter.toLowerCase())
            return false;
        if (onlyWithPhoto && user.picture_large === "-")
            return false;
        if (onlyFavorites && !user.favorite)
            return false;
        return true;
    });
...
(_b = document.getElementById("applyFilters")) === null || _b === void 0 ? void 0 : _b.addEventListener("click", filterUsers);
// Відкриття попапу з інформацією про викладача
...
(_c = document.querySelector(".close")) === null || _c === void 0 ? void 0 : _c.addEventListener("click", closeModal);
// Закриття попапу при натисканні поза вікном
...
// Функція сортування з індикацією стрілкою
let sortDirection = {};
const sortTable = (field, type) => {
    // Сортуємо копію масиву користувачів
    ...
    sortedUsers.sort((a, b) => {
        let valueA = a[field];
        let valueB = b[field];
        // Перевірка типу для правильного порівняння
        if (type === "string") {
    ...
    // Оновлюємо таблицю з відсортованими користувачами
    renderTable(sortedUsers);
    // Оновлюємо стрілки сортування в заголовках таблиці
    updateSortArrows(field);
};
// Функція для рендеру таблиці
const renderTable = (filteredUsers = users) => {
    const tableBody = document.querySelector("table tbody");
    if (!tableBody)
        return;
    // Очищаємо таблицю перед рендером нових даних
    tableBody.innerHTML = "";
...
// Оновлення стрілок у заголовках
const updateSortArrows = (field) => {
    const headers = document.querySelectorAll("th");
    headers.forEach(header => {
        const sortIndicator = header.querySelector(".sort-arrow");
        if (sortIndicator) {
            sortIndicator.remove(); // Видаляємо стару стрілку
        }
        // Додаємо нову стрілку до заголовка
        if (header.id === `sort-${field}`) {
            const arrow = document.createElement("span");
            arrow.classList.add("sort-arrow");
            arrow.textContent = sortDirection[field] === 'asc' ? "⬆" : "⬇";
            header.appendChild(arrow);
        }
    });
};
// Обробники подій для заголовків таблиці
(_d = document.getElementById("sort-name")) === null || _d === void 0 ? void 0 : _d.addEventListener("click", () => sortTable("full_name", "string"));
(_e = document.getElementById("sort-course")) === null || _e === void 0 ? void 0 : _e.addEventListener("click", () => sortTable("course", "string"));
(_f = document.getElementById("sort-age")) === null || _f === void 0 ? void 0 : _f.addEventListener("click", () => sortTable("age", "number"));
(_g = document.getElementById("sort-country")) === null || _g === void 0 ? void 0 : _g.addEventListener("click", () => sortTable("country", "string"));
// Форма пошуку
const searchForm = document.getElementById("searchForm");
const searchInput = document.getElementById("searchInput");
// Функція для пошуку
const searchUsers = (query) => {
    const searchTerm = query.toLowerCase(); // Перетворюємо запит на малий регістр
    // Фільтруємо користувачів за ім'ям, коментарем та віком
    const filteredUsers = users.filter((user) => {
        const fullName = `${user.title} ${user.full_name}`.toLowerCase();
        const note = user.note ? user.note.toLowerCase() : "";
        const age = user.age.toString();
        // Перевірка на наявність пошукового терміну в полях ім'я, коментар або вік
        return fullName.includes(searchTerm) || note.includes(searchTerm) || age.includes(searchTerm);
    });
    // Виводимо відфільтрованих користувачів
    renderUsers(filteredUsers);
};
// Додаємо обробник події на форму для пошуку
searchForm.addEventListener("submit", (event) => {
    event.preventDefault(); // Не дозволяємо перезавантаження сторінки
    const query = searchInput.value.trim();
    searchUsers(query);
});
// Оновлена функція рендеру карток користувачів
const renderUsers = (filteredUsers = users) => {
    ...
    if (container) {
        ...
        filteredUsers.forEach((user) => {
            ...
            userDiv.className = "teacher-card";
            userDiv.innerHTML = `
                <img src="${user.picture_large}" alt="${user.full_name}" class="teacher-img">
                <h3>${user.title} ${user.full_name}</h3>
                <p>${user.course}</p>
                <p>${user.country}</p>
                <button class="favorite-button">${user.favorite ? "★" : "☆"}</button>
            `;
            userDiv.addEventListener("click", () => openModal(user));
            const favoriteButton = userDiv.querySelector(".favorite-button");
            favoriteButton === null || favoriteButton === void 0 ? void 0 : favoriteButton.addEventListener("click", (event) => {
    ...
    // Рендеримо дані в таблиці
    renderUsersTable(filteredUsers);
};
// Оновлена функція рендеру для таблиці
const renderUsersTable = (filteredUsers = users) => {
    const tableBody = document.querySelector("#teachers-table tbody");
    if (!tableBody) {
        console.error("Елемент з id 'teachers-table' не знайдено!");
        return;
    }
    // Очищаємо існуючі рядки таблиці
    tableBody.innerHTML = "";
    filteredUsers.forEach((user) => {
        const row = document.createElement("tr");
        const nameCell = document.createElement("td");
        nameCell.textContent = `${user.title} ${user.full_name}`;
        const courseCell = document.createElement("td");
        courseCell.textContent = user.course;
        const ageCell = document.createElement("td");
        ageCell.textContent = `${user.age}`;
        const genderCell = document.createElement("td");
        genderCell.textContent = user.gender.charAt(0).toUpperCase() + user.gender.slice(1);
...
renderUsers();
// Функція рендеру улюблених викладачів
const renderFavorites = () => {
    const favoritesList = document.getElementById("favorites-list");
    // Очищаємо список перед рендером нових даних
    favoritesList.innerHTML = "";
    // Фільтруємо користувачів, щоб показати тільки улюблених
    const favoriteUsers = users.filter(user => user.favorite);
    // Створюємо елементи для кожного улюбленого викладача
    favoriteUsers.forEach(user => {
        const favoriteItem = document.createElement("div");
        favoriteItem.classList.add("favorite-item");
        favoriteItem.innerHTML = `
            <img src="${user.picture_thumbnail}" alt="${user.full_name}">
            <h2>${user.full_name}</h2>
            <p>${user.country}</p>
        `;
        // Додаємо елемент у список улюблених
        favoritesList.appendChild(favoriteItem);
    });
};
// Оновлений метод для додавання/видалення викладачів в обране
const toggleFavorite = (user) => {
    user.favorite = !user.favorite;
    renderUsers(); // Оновлюємо всі картки
    renderFavorites(); // Оновлюємо список улюблених викладачів
};
// Відображення улюблених викладачів після початкової завантаження
renderFavorites();
// Секція для форми додавання викладача
const addTeacherPopup = document.querySelector(".pop-add");
const addButton = document.querySelector(".add-button");
const closePopupButton = document.querySelector(".pop-add .close");
// Функція для відкриття форми додавання викладача
const openAddTeacherPopup = () => {
    addTeacherPopup.style.display = "block";
};
// Функція для закриття форми додавання викладача
const closeAddTeacherPopup = () => {
    addTeacherPopup.style.display = "none";
};
// Обробник для закриття форми при натисканні на хрестик
closePopupButton === null || closePopupButton === void 0 ? void 0 : closePopupButton.addEventListener("click", closeAddTeacherPopup);
// Функція для збору даних з форми і додавання нового викладача
const addNewTeacher = () => {
    var _a;
    ...
    const sex = (_a = document.querySelector('input[name="sex"]:checked')) === null || _a === void 0 ? void 0 : _a.value;
    const bgColor = document.getElementById("bg-color").value;
    const notes = document.getElementById("notes").value;
    // Створюємо нового викладача
    const newTeacher = {
        gender: sex || "male", // Якщо не вибрано, ставимо за замовчуванням "male"
        title: "Mr.", // Можна додати більше логіки для вибору титулу
        full_name: name,
        city: city,
        state: "",
        country: country,
        postcode: "",
        coordinates: { latitude: "0", longitude: "0" }, // Можна додати логіку для координат
        timezone: { offset: "+00:00", description: "UTC" }, // Можна уточнити час
        email: email,
        b_date: dob,
        age: new Date().getFullYear() - new Date(dob).getFullYear(), // Підрахунок віку на основі дати народження
        phone: phone,
        picture_large: "https://via.placeholder.com/150", // Замість зображення можна використовувати аватар
        picture_thumbnail: "https://via.placeholder.com/50",
        id: `${Math.random()}`, // Генерація випадкового id для користувача
        favorite: false,
        course: speciality, // Спеціальність буде курсом викладача
        bg_color: bgColor, // Колір фону
        note: notes || null // Якщо є примітки
    };
    // Додаємо нового викладача в масив користувачів
    users.push(newTeacher);
    // Оновлюємо відображення списку викладачів
    renderUsers();
    renderFavorites(); // Оновлюємо також список улюблених
    // Закриваємо форму
    closeAddTeacherPopup();
};
// Обробник події для кнопки "Add"
addButton === null || addButton === void 0 ? void 0 : addButton.addEventListener("click", addNewTeacher);
// Функція для відображення форми додавання викладача
const showAddTeacherForm = () => {
    openAddTeacherPopup();
};
// Додавання кнопки на сторінці для відкриття форми
const openFormButton = document.createElement("button");
openFormButton.textContent = "Add Teacher";
openFormButton.addEventListener("click", showAddTeacherForm);
document.body.appendChild(openFormButton);
// Ініціалізуємо відображення форми додавання викладача
const initAddTeacherPopup = () => {
    addTeacherPopup.style.display = "none"; // Початково форма буде прихована
};
// Викликаємо функцію ініціалізації форми
initAddTeacherPopup();
