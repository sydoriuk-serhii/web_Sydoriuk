...
// Оголошення інтерфесу, будем юзать далі
...
// Відображаємо вчителя з рандом предметом
...
const users: User[] = randomUserMock.map(normalizeUser);
...
const filterUsers = (): void => {
    ...
    let filteredUsers = users.filter((user: User) => {
        if (ageFilter === "18-31" && !(user.age >= 18 && user.age <= 31)) return false;
        if (ageFilter === "32-45" && !(user.age >= 32 && user.age <= 45)) return false;
        if (ageFilter === "46+" && !(user.age >= 46)) return false;
        ...
        if (regionFilter !== "Other" && !regions[regionFilter].includes(user.country)) return false;

        if (sexFilter !== "Other" && user.gender !== sexFilter.toLowerCase()) return false;
        if (onlyWithPhoto && user.picture_large === "-") return false;
...
// Попап з інфою про вчителів
const openModal = (user: User): void => {
    const modal = document.getElementById("teacherModal") as HTMLDivElement;
    const modalImage = document.getElementById("modalImage") as HTMLImageElement;
    const modalName = document.getElementById("modalName") as HTMLHeadingElement;
    const modalSubject = document.getElementById("modalSubject") as HTMLSpanElement;
    const modalCountry = document.getElementById("modalCountry") as HTMLSpanElement;
    const modalAge = document.getElementById("modalAge") as HTMLSpanElement;
    const modalEmail = document.getElementById("modalEmail") as HTMLSpanElement;
    const modalNumber = document.getElementById("modalNumber") as HTMLSpanElement;
...
const sortTable = (field: keyof User, type: "string" | "number"): void => {
...
const renderTable = (filteredUsers: User[] = users): void => {
...
const updateSortArrows = (field: keyof User): void => {
...
document.getElementById("sort-name")?.addEventListener("click", () => sortTable("full_name", "string"));
document.getElementById("sort-course")?.addEventListener("click", () => sortTable("course", "string"));
document.getElementById("sort-age")?.addEventListener("click", () => sortTable("age", "number"));
document.getElementById("sort-country")?.addEventListener("click", () => sortTable("country", "string"));
...
const renderUsers = (filteredUsers: User[] = users): void => {
    const container = document.getElementById("users-container");
    if (container) {
        container.innerHTML = "";
        filteredUsers.forEach((user) => {
            ...
            const favoriteButton = userDiv.querySelector(".favorite-button") as HTMLButtonElement;
...
const renderUsersTable = (filteredUsers: User[] = users): void => {
...
const toggleFavorite = (user: User): void => {
...
// Додавання вчителя
const addTeacherPopup = document.querySelector(".pop-add") as HTMLDivElement;
const addButton = document.querySelector(".add-button") as HTMLButtonElement;
const closePopupButton = document.querySelector(".pop-add .close") as HTMLSpanElement;

const openAddTeacherPopup = (): void => {
    addTeacherPopup.style.display = "block";
};

const closeAddTeacherPopup = (): void => {
    addTeacherPopup.style.display = "none";
};

closePopupButton?.addEventListener("click", closeAddTeacherPopup);

const addNewTeacher = (): void => {
    const name = (document.getElementById("name") as HTMLInputElement).value;
    const speciality = (document.getElementById("speciality") as HTMLInputElement).value;
    const country = (document.getElementById("country") as HTMLInputElement).value;
    const city = (document.getElementById("city") as HTMLInputElement).value;
    const phone = (document.getElementById("phone") as HTMLInputElement).value;
    const email = (document.getElementById("email") as HTMLInputElement).value;
    const dob = (document.getElementById("dob") as HTMLInputElement).value;
    const sex = (document.querySelector('input[name="sex"]:checked') as HTMLInputElement)?.value;
    ...
    const newTeacher: User = {
        ...
        phone: phone,
        picture_large: "https://via.placeholder.com/150",
...
addButton?.addEventListener("click", addNewTeacher);
...
const initAddTeacherPopup = (): void => {
    addTeacherPopup.style.display = "none";
};
