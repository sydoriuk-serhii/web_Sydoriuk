import { users, formatRandomUser } from "./lab3.js";

console.log(users);
export let filteredUsers = [...users];
...
// Task 1
export function renderTeachers(userList) {
...
document.getElementById("stat-table-header").addEventListener("click", (e) => {
    ...
    sortedStatUsers = sortUsersBy(users, statSortBy, statSortOrder);
    currentPage = 1;
...
document.querySelector(".add-teacher-popup-form").addEventListener("submit", function (e) {
    ...
    const gender = document.querySelector("input[name='gender']:checked")?.id || "not defined";
    ...
    users.push(formatRandomUser(newUser, users.length + 1));
    filteredUsers = [...users];
